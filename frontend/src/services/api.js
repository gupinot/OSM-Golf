...
export async function searchByName(query, { fresh = false } = {}) {
  ...
  const res = await apiFetch(`/api/search/name?${params}`);
...
export async function searchByZone({ lat, lng, city, radius, fresh = false }) {
  ...
  if (fresh) params.set('fresh', '1');
  const res = await apiFetch(`/api/search/zone?${params}`);
  if (!res.ok) throw new Error((await res.json()).error);
  return res.json();
}
...
export async function fetchZoneStats(lat, lng, radius, { fresh = false } = {}) {
  ...
  if (fresh) params.set('fresh', '1');
  const res = await apiFetch(`/api/search/zone-stats?${params}`);
  if (!res.ok) throw new Error((await res.json()).error);
  return res.json();
}
...
export async function fetchHoles(osmId, lat, lng, radius = 5) {
  ...
  const res = await apiFetch(`/api/holes?${params}`);
  if (!res.ok) throw new Error((await res.json()).error);
  return res.json();
}
...
export async function fetchCgolfHoles(osmId, name, lat, lng) {
  ...
  const res = await apiFetch(`/api/cgolf-holes?${params}`);
  if (!res.ok) throw new Error((await res.json()).error);
  return res.json();
}

export async function analyzeCustomScorecard(payload) {
  const res = await apiFetch('/api/cgolf-holes/analyze', {
    method: 'POST',
...
export async function fetchPersistedCustomSources(osmId) {
  ...
  const res = await apiFetch(`/api/cgolf-holes/custom-sources?${params}`);
  if (!res.ok) return {};
  return res.json();
}
...
export async function removePersistedCustomSource(osmId, courseKey) {
  ...
  await apiFetch(`/api/cgolf-holes/custom-source?${params}`, { method: 'DELETE' });
}
